  ...
  switch (action.type) {
    case 'ADD_TO_CART': {
      ...
      if (action.payload.counterType === '+') {
        ...
        newState.selectedItems.items = newState.selectedItems.items.filter(item => item.order_id !== action.payload.orderId);
      }
    ...
    case "UPDATE_ITEM_IN_CART":{
      let index = newState.selectedItems.items.findIndex((item) => item.order_id === action.payload.orderId);
...
export const modifyItemInCartThunk = (item, number, counterType, restaurantId) => (dispatch, getState) => {
  ...
  if(getState().cartReducer.selectedItems.items.length && getState().cartReducer.selectedItems.restaurantId !== item.restaurant_id){
    axios
    .delete(`https://food-menu-app-backend.herokuapp.com/cart/checkOutCartItems/${userId}`)
    .catch(err => {
      console.log(err);
      throw err;
    })
  ...
  if(counterType === '+'){
    ...
    for(let i = 0; i < cartItems.length && (cartIsEmpty !== 1); i++){
      if(cartItems[i].dish_id === item.id && _.isEqual(cartItems[i].addons, item.addons)){
        console.log('duplicate found in cart');
        axios.post(`https://food-menu-app-backend.herokuapp.com/dish/increaseQuantityInCart/${cartItems[i].order_id}`)
          .then(() => {
            dispatch({
    ...
    axios
      ...
      .then((res) => {
        axios.get(`https://food-menu-app-backend.herokuapp.com/cart/getOrderItemDetail/${userId}/${res.data.insertId}`)
          .then((res) => {
    ...
    axios
      .delete(`https://food-menu-app-backend.herokuapp.com/cart/deleteFromCart/${userId}/${item.order_id}`)
      .then(() => {
      ...
      .then(res => {
        if(res.data.data.length){
          dispatch({
            type: 'LOAD_CART',
            payload: {
              selectedItems: {items:res.data.data, restaurantId: res.data.data[0].restaurant_id}
            }
          })
        }
      })
...
export const increaseQuantityInCartThunk = (orderId) => (dispatch, getState) => {
  ...
  const userId = getState().userReducer.userId;
  axios.post(`https://food-menu-app-backend.herokuapp.com/dish/increaseQuantityInCart/${orderId}`)
  .then(() => {
    dispatch({
      type: 'UPDATE_ITEM_IN_CART',
      payload: {
...
export const decreaseQuantityInCartThunk = (orderId) => (dispatch, getState) => {
  ...
  const userId = getState().userReducer.userId;
  axios.post(`https://food-menu-app-backend.herokuapp.com/cart/decreaseQuantityInCart/${orderId}`)
  .then(() => {
    dispatch({
