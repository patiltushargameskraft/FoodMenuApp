...
import { Divider } from 'react-native-elements';
import { Searchbar } from 'react-native-paper';
>>>>>>> c0f6921846f5d12aa830c7883745732ff40e75ca
...
export default function SearchBar({route, navigation}) {
  ...
  const placeholders = ['for Dishes', 'Dishes by Cuisine', 'Dishes by Categories', 'Dishes by Description', 'for Restaurants']
  const inResPlaceholders = ['for Dishes', 'for Cuisines', 'for Categories', 'for Description']
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#eee', flex: 1}}>
     <Searchbar
      placeholder={(resId ? `Search in ${resName} ${inResPlaceholders[searchType]}`: `Search ${placeholders[searchType]}`)}
